<#
.SYNOPSIS
Gets useful information about EC2 instances to create an Ansible inventory file

.DESCRIPTION
Returns the instance Name (if name tag exists), Private IP address and platform 

.PARAMETER ProfileName
Specifies an AWS PowerShell Credential Profile to use when executing this
script.
Using template from: https://app.pluralsight.com/library/courses/aws-tools-windows-powershell-automating-cloud-operations/table-of-contents

.PARAMETER Region
Specifies a region to use when executing this script.
#>

function Get-EC2ForAnsibleInventory {
  [CmdletBinding()]
  Param (
    [Parameter()]
  ...
  Process {
    ...
    $instanceList = $ec2List.instances
    ...
    foreach ($instance in $instanceList) {
      ...
      $ec2 = New-Object -TypeName PSObject -Property $props
      $ec2
    }
  }
